...
//  Neighbors
//
//  Created by Diana Chen on 6/21/16.
//  Copyright © 2016 Pocoa. All rights reserved.
//

import UIKit

class PlayController: NSObject, SPTAudioStreamingPlaybackDelegate, SPTAudioStreamingDelegate {
    
    class var sharedPlayer: PlayController  {
        struct Singleton {
            static let instance = PlayController()
        }
    ...
    func queueSpotifyFreeSample(completed: (success: Bool) -> Void) {
        if let trackURI = NSURL(string: "spotify:track:1IkCG9jkzCrl3TadPTh4dU"),
            track2URI = NSURL(string: "spotify:track:1IkCG9jkzCrl3TadPTh4dU") {
